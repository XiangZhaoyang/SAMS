...
$(function($) {
	...
	$qIdNd.on('click', function(event) {
		...
		$qDepartmentHintNd.removeClass('showInline').addClass('hide');
	...
	//根据院系查询
	$qDepartmentNd.on('click', function(event) {
		event.preventDefault();
		$qIdHintNd.removeClass('showInline').addClass('hide');
		$qCNameHintNd.removeClass('showInline').addClass('hide');
		$qDepartmentHintNd.removeClass('hide').addClass('showInline');
		$req = $.ajax({
	...
	$qIdBtnNd.on('click', function(event) {
		event.preventDefault();
		var sId = $('#sId').val(),
			url = '/api/department/' + sId,
			rstr = '',
			data,
			dataArr = [];
		if (!sId) {
			showHintBox($qResultHint);
			showHint($hintText,'请输入要查询课程的id');
			return;
		}
		$req = $.ajax({
			url: url,
	...
	function showRt(qRData){
		...
		qRData.forEach(function(item,index){
			var data = item,
				qO = {},
				qData = qO.data = {},
				sId = qData['course_id'] = data['course_id'],
				sName = qData['course_name'] = data['course_name'],
				steacher = qData['course_teacher_id'] =data['course_teacher_id'],
				sclasses = qData['course_course_id'] = data['course_course_id'],
				sDepart = qData['course_department_id'] = data['course_department_id'],
				sBelong = qData['course_belong'] =data['course_belong'],
				sStyle = qData['course_style'] = data['course_style'],
				sCredit = qData['course_credit'] = data['course_credit'],
				sYear = qData['course_year'] = data['course_year'],
				sTerm = qData['course_term'] = data['course_term'],
				sInf = qData['course_information'] = data['course_information'];
			qO.change = false;
			rstr += '<tr class="tableContentItem"><td><span class="course_id">' + sId +
				'</span></td><td><input class="course_name" item="' + index + 
				'" type="text" name="" value="'+ sName +
				'"></td><td><input class="student_sex" item="' + index + 
				'" type="text" name="" value="' + sSex +
				'"></td><td><input class="course_teacher_id" item="' + index + 
				'" type="text" name="" value="'+ steacher +
				'"></td><td><input class="course_classes_id" item="' + index +
				'" type="text" name="" value="'+ sAdress + 
				'"></td><td><input class="course_department_id" item="' + index +
				'" type="text" name="" value="' + sPhone + 
				'"></td><td><input class="course_belong" item="' + index + 
